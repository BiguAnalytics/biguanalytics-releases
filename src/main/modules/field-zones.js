...
const FOUR_SECTOR_FIELD_ZONES = Object.freeze([
...
const FOUR_SECTOR_ZONE_IDS = new Set(FOUR_SECTOR_FIELD_ZONES.map(zone => zone.id));
 ...
 * @returns {object|null}
 */
function mapLegacyZoneToFourSector(zone) {
 ...
 */
function normalizeFieldZone(zone) {
 ...
 * @returns {boolean}
 */
function isFourSectorZoneId(zone) {
  const normalized = normalizeFieldZone(zone);
  return Boolean(normalized && FOUR_SECTOR_ZONE_IDS.has(normalized.id));
}
